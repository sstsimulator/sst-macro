 ...
 */
int
MTestGetIntercomm(MPI_Comm *comm, int *isLeftGroup, int min_size)
{
    ...
    *isLeftGroup = 0;
    ...
    case 0:
      ...
      if (size > 1)
      {
        ...
        *isLeftGroup = rank < size / 2;
        merr = MPI_Intercomm_create(mcomm, 0, MPI_COMM_WORLD, rleader, 12345,
            comm);
        if (merr)
          MTestPrintError(merr);
    ...
    case 1:
      ...
      if (size > 1)
      {
        ...
        *isLeftGroup = rank == 0;
        merr = MPI_Intercomm_create(mcomm, 0, MPI_COMM_WORLD, rleader, 12346,
            comm);
        if (merr)
          MTestPrintError(merr);
    ...
    case 2:
      ...
      if (size > 3)
      {
        ...
        *isLeftGroup = rank < 2;
        merr = MPI_Intercomm_create(mcomm, 0, MPI_COMM_WORLD, rleader, 12347,
            comm);
        if (merr)
          MTestPrintError(merr);
