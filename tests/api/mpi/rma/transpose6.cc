...
#define NCOLS 100
  ...
  transpose6(int argc, char *argv[])
  {
    ...
    if (nprocs != 1)
    {
      MPI_Comm_split(MPI_COMM_WORLD, (rank < 1), rank, &world);
    }
    else
    {
    ...
    if (rank < 1)
    {
      ...
      /* create datatype for one column */MPI_Type_vector(NROWS, 1, NCOLS,
          MPI_INT, &column);
      /* create datatype for matrix in column-major order */MPI_Type_hvector(
          NCOLS, 1, sizeof(int), column, &xpose);
      MPI_Type_commit(&xpose);

      MPI_Win_create(B, NROWS * NCOLS * sizeof(int), sizeof(int),
