...
namespace getgroup
{
  ...
  getgroup(int argc, char *argv[])
  {
    ...
    while (MTestGetIntracommGeneral(&comm, minsize, 1))
    {
      ...
      MPI_Win_create(buf, sizeof(int) * 10, sizeof(int), MPI_INFO_NULL, comm,
          &win);
      ...
      if (result != MPI_IDENT)
      {
        errs++;
        fprintf(stderr,
            "Group returned by Win_get_group not the same as the input group\n");
      }
