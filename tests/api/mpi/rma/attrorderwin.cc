...
namespace attrorderwin
{
  ...
  static char
      MTestDescrip[] =
          "Test creating and inserting attributes in \
different orders to ensure that the list management code handles all cases.";

  int
  checkAttrs(MPI_Win win, int n, int key[], int attrval[]);
  int
  checkNoAttrs(MPI_Win win, int n, int key[]);

  int
  attorderwin(int argc, char *argv[])
  {
    int errs = 0;
    int key[3], attrval[3];
    int i;
