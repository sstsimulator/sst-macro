...
namespace mixedsync
{
  ...
  mixedsync(int argc, char *argv[])
  {
    ...
    /* Allocate the window buffer and create the memory window. */MPI_Alloc_mem(
        count * sizeof(int), MPI_INFO_NULL, &winbuf);
    if (!winbuf)
    {
    ...
    for (loop = 0; loop < 2; loop++)
    {
      ...
      if (crank == source)
      {
        ...
        MPI_Put(buf2, count2, MPI_INT, dest, count0 + count1, count2, MPI_INT,
            win);
    ...
    for (loop = 0; loop < 2; loop++)
    {
      ...
      if (crank == source)
      {
        ...
        MPI_Accumulate(buf0, count0, MPI_INT, dest, 0, count0, MPI_INT,
            MPI_SUM, win);
        MPI_Accumulate(buf1, count1, MPI_INT, dest, count0, count1, MPI_INT,
            MPI_SUM, win);
        MPI_Put(buf2, count2, MPI_INT, dest, count0 + count1, count2, MPI_INT,
            win);
        MPI_Win_unlock(dest, win);
    ...
    for (loop = 0; loop < 2; loop++)
    {
      /* Perform several communication operations, mixing synchronization
       ...
       optimization that may be present. */
      MTestPrintfMsg(3,
          "Begining loop %d of mixed sync put/get/acc operations\n", loop);
      MPI_Barrier(comm);
      if (crank == source)
      {
        MPI_Win_lock(MPI_LOCK_EXCLUSIVE, dest, 0, win);
