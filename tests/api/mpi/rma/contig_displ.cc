...
namespace contig_displ
{
  ...
  int
  contig_displ(int argc, char **argv)
  {
    ...
    if (nprocs != 1)
    {
      MPI_Comm_split(MPI_COMM_WORLD, (rank == 0), 0, &world);
    }
    else
    {
    ...
    if (rank == 0)
    {
      ...
      /* To improve reporting of problems about operations, we
       change the error handler to errors return */MPI_Comm_set_errhandler(world,
          MPI_ERRORS_RETURN);
      ...
      /* To improve reporting of problems about operations, we
       change the error handler to errors return */MPI_Win_set_errhandler(win,
          MPI_ERRORS_RETURN);
      ...
      mpi_err = MPI_Get(&getval, 1, MPI_INT, 0, 1, 1, type, win);
      if (mpi_err != MPI_SUCCESS)
        goto err_return;
    ...
    err_return: printf("MPI function error returned an error\n");
