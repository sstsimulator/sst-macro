...
namespace fkeyvalwin
{
  ...
  static char
      MTestDescrip[] =
          "Test freeing keyvals while still attached to \
...
executed";
  ...
  int
  copy_fn(MPI_Win oldwin, int keyval, void *extra_state,
  ...
  int
  delete_fn(MPI_Win win, int keyval, void *attribute_val, void *extra_state)
  {
  ...
  fkeyvalwin(int argc, char *argv[])
  {
    ...
    while (MTestGetWin(&win, 0))
    {
      ...
      /* See MPI-1, 5.7.1.  Freeing the keyval does not remove it if it
       is in use in an attribute */MPI_Win_free_keyval(&keyval);
