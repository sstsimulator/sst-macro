...
#define NTIMES 20  /* no of times each process calls the counter
  ...
  void
  Get_nextval_tree(MPI_Win win, int *get_array, MPI_Datatype get_type,
  ...
  int
  fetchandadd_tree_am(int argc, char *argv[])
  {
    int rank, nprocs, i, *counter_mem, *get_array, *get_idx, *acc_idx, mask,
        nlevels, level, idx, tmp_rank, pof2;
    ...
    if (rank == 0)
    {
      ...
      /* free(counter_mem) */MPI_Free_mem(counter_mem);
    ...
    else
    {
      ...
      for (i = 0; i < NTIMES; i++)
      {
        Get_nextval_tree(win, get_array, get_type, acc_type, nlevels,
            counter_vals + i);
      ...
      /* gather the results to the root */MPI_Gather(counter_vals, NTIMES,
          MPI_INT, NULL, 0, MPI_DATATYPE_NULL, 0, MPI_COMM_WORLD);
      free(counter_vals);
    }
