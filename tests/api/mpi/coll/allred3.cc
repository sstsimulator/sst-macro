 ...
 */
#define MAXCOL 256
static global_int matSize(0);  /* Must be < MAXCOL */
static global_int max_offset(0);
...
int allred3( int argc, char *argv[] )
{
    int size, rank;
    int minsize = 2, count; 
    MPI_Comm      comm;
	...
	int e = isIdentity( comm, bufout );

	if(e == 0){
	  printf("rank %d passed first allreduce", rank);
	}
