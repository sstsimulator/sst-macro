...
However, an \inlinecode{enum} type requires all possible values to be declared at once in a single header file.
\sprockit leverages initialization of static variables to allow a flexible enum type called \inlinecode{expandable_enum}.
First an \inlinecode{enum} type is declared in a header file, for example in \inlinecode{sst_message}
...
Because of C++ initialization standards, all enums will have unique values assigned once the program enters \inlinecode{main}.
