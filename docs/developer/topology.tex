...
\begin{itemize}
\item Source and destination node addresses
\item \inlinecode{path} object (see below)
\item Routing header
\item Payload derived from the original flow
\item Flow ID - globally unique identifier for the flow
\item The number of bytes in the packet
\end{itemize}
...
Switches implement \emph{contention modeling} while routers implement \emph{path computation}.
...
the packet provides a bucket of bits (\inlinecode{header_metadata_}) that can be used by the router.
