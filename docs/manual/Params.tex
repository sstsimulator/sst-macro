...
\hline
\end{tabular}
\subsubsection{Namespace ``node.nic.buffer''}
\openTable
\hline
...
\hline
\end{tabular}
\subsubsection{Namespace ``node.buffer.delay\_histogram''}
\input{histogramDescr}
\subsubsection{Namespace ``node.buffer.congestion\_spyplot''}
\input{spyplotDescr}
...
\openTable
\hline
model \paramType{string} & pisces & pisces & The type of netlink congestion model to use \\
\hline
...
\openTable
\hline
name \paramType{string} & minimal & minimal, valiant, ugal, fat\_tree & The name of the routing algorithm to use for routing packets. \\
\hline
