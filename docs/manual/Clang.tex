...
By default, global variables in a header file area NOT redirected unless explicitly specified in a header configuration file.
...
All temporary, intermediate source files will not be deleted at the end of compilation.
...
Can only apply to a binary equal operator assigning a value.
...
The pragma causes the skeleton to simply assign \inlinecode{destAddr} to \inlinecode{nullptr}.

\subsection{pragma sst return [new\_value:C++ expression]}
Equivalent to \inlinecode{pragma sst init}. Replaces the target of a return statement with the given expression.
...
For example, all MPI calls have their input buffers converted to null pointers to indicate a simulated MPI call.
If the MPI call should be emulated and actually deliver a real payload, the MPI call must be explicitly marked with \inlinecode{pragma keep}.
...
More control over whether a transformation is skipped can be provided by the \inlinecode{keep_if} program.
An example can be found in CoMD.
...
Applied to functions. The function prototype is kept, but an empty body is inserted instead of the actual code.
...
\inlinecode{pragma sst new} simply turns off a given memory allocation setting it a null value.
...
When this occurs, a compiler error is throw flagging where the ambiguity occurs.
