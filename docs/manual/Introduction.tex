...
The following sections describe the state of the software API's (found in \folder{sstmac}) that are available in \sstmacro for use by applications, as of this release. The level of testing indicates 
...
Functions that are not implemented will throw a \class{unimplemented\_error}, reporting the function name. 
...
\begin{itemize}
\item{Sockets:} The Socket API is mostly implemented. Most basic client/server functionality is available.  However, only the default socket options are allowed. In most cases, \inlinecode{setsockopt} is just a no-op.
\item{Pthreads:}  Only the \func{pthread\_create()}, \func{pthread\_join()}, and \func{pthread\_self()} along with mutex and condition variables are implemented.  A basic  pthread test validates the core spawn/run/join behavior.
\end{itemize}
...
\begin{itemize}
\item Anything using or having to do with Inter-communicators (\func{MPI\_Intercomm\_create()})
...
\item Changing the name of built-in datatypes with \func{MPI\_Type\_set\_name()} (MPI test 171).
\item \func{MPI\_Create\_darray()}, \func{MPI\_Create\_subarray()}, and \func{MPI\_Create\_resized()}
\item \func{MPI\_Pack\_external() }, which is only useful for sending messages across MPI implementations apparently.
\item \func{MPI\_Type\_match\_size() }  - extended fortran support
...
No \func{MPI\_Info\_*}, \func{MPI\_*\_keyval}, or \func{MPI\_Attr\_*} functions are supported.

\subsubsection{Point-to-Point}
\label{subsubsec:issues:mpi:ptpt}

\begin{itemize}
\item \func{MPI\_Grequest\_*} functions (generalized requests).
...
\begin{itemize}
\item There seems to be a problem with using MPI\_FLOAT and MPI\_PROD in \func{MPI\_Allreduce()} (MPI test 22)
\item There seems to be a problem with using non-commutative user-defined operators in \func{MPI\_Reduce()} and \func{MPI\_Allreduce()}.
\item \func{MPI\_Alltoallw()} is not implemented
\item \func{MPI\_Exscan()} is not implemented
\item \func{MPI\_Reduce\_Scatter\_block()} is not implemented.
\item \func{MPIX\_*} functions are not implemented  (like non-blocking collectives).
\item Calling MPI functions from user-defined reduce operations (MPI test 39; including \func{MPI\_Comm\_rank}).
\end{itemize}
...
\begin{itemize}
\item \func{MPI\_Is\_thread\_main()} is not implemented.
\end{itemize}
...
For primitives and primitive arrays, refer to \inlinecode{<sstmac/openshmem/shmem/globals.h>} for replacing types, e.g. the following code
