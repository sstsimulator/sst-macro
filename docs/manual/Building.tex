...
\item Run the \inlineshell{sstmac} wrapper Python script that runs SST/macro-specific parameter OR
\item Write a custom Python script 
\end{itemize}

\subsubsection{Build SST core}
...
\subsubsection{Build SST/macro element library}
...
Full details on building SST Python scripts can be found at \url{http://sst-simulator.org/SSTPages/SSTMainDocumentation}.  To preserve the old parameter format in the near-term, SST/macro provides the \inlineshell{sstmac} script:
...
options="$@"
$SST_PREFIX/sst $SSTMAC_PREFIX/include/python/default.py --model-options="$options"
\end{ViFile}
...
\subsection{Running SST/macro Parameter Files}
