...
\subsection{Boost 1.65 and fcontext (Beta)}\label{subsubsec:boost}
...
You will need a \inlineshell{user-config.jam} to configure the Boost compiler flags.
You then need to bootstrap, compile, and install the prerequisite Boost libraries. 
For GCC, the \inlineshell{user-config.jam} should go in the top-level home directory and the file should contain the line:
...
./b2 ---prefix=$INSTALL
\end{ViFile}
The toolset can be changed to use gcc by replacing clang with gcc in each of the \inlineshell{user-config.jam} and run commands.
