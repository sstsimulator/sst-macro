...
DUMPI can be used as it's own library within the \sstmacro source tree by changing to \folder{sst-macro/sst-dumpi}, 
...
To get DUMPI by itself, either copy the \folder{sstmacro/dumpi} directory somewhere else or visit \url{https://github.com/sstsimulator/sst-dumpi} and follow similar instructions for obtaining \sstmacro.
...
To demonstrate how an application is run in \sstmacro, we'll use a very simple send-recv program located in \folder{skeletons/sendrecv}.
...
We recommend structuring the Makefile for your project like the one seen in \folder{skeletons/sendrecv/Makefile} :
