...
launch_app1_cmd = aprun -n 8
launch_indexing = block
launch_allocation = cartesian
launch_cart_sizes = 2 2 2
launch_cart_offsets = 0 0 0
\end{ViFile}
...
(\inlineshell{launch_cart_sizes}) and which octant (\inlineshell{launch_cart_offsets}).
...
launch_indexing = block
launch_allocation = cartesian
launch_cart_sizes = 2 2 1 2
launch_cart_offsets = 0 0 0 0
\end{ViFile}

We allocate a set of switches across an XY plane (2 in X, 2 in Y, 1 in Z for a single plane).
The last entry in \inlineshell{launch_cart_sizes} indicates that both nodes on each switch should be used.
