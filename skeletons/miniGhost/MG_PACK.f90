    ...
    !  REAL(KIND=MG_REAL), DIMENSION(0:NX+1, 0:NY+1, 0:NZ+1), INTENT(IN) :: &
     !    GRID
      ...
      IF ( linfo(mype+1)%NEIGHBORS(BACK) /= -1 ) THEN
         ...
         DO J = 0, NY+1
            DO I = 0, NX+1
               linfo(mype+1)%COUNT_SEND_BACK = linfo(mype+1)%COUNT_SEND_BACK + 1
              ! linfo(mype+1)%SEND_BUFFER_BACK ( linfo(mype+1)%COUNT_SEND_BACK ) = GRID ( I, J, 1 )
            END DO
         END DO
         MG_PERF(mype+1)%TIME_PACK_Z_PE = MG_PERF(mype+1)%TIME_PACK_Z_PE + MPI_WTIME() - &
                                  TIME_START_DIR
      	call SSTMAC_compute_loop2(0, NX+1, 0, NY+1, 2)
      END IF
      ...
      IF ( linfo(mype+1)%NEIGHBORS(FRONT) /= -1 ) THEN
         ...
         DO J = 0, NY+1
            DO I = 0, NX+1
               linfo(mype+1)%COUNT_SEND_FRONT = linfo(mype+1)%COUNT_SEND_FRONT + 1
              ! linfo(mype+1)%SEND_BUFFER_FRONT ( linfo(mype+1)%COUNT_SEND_FRONT ) = GRID ( I, J, NZ )
            END DO
         END DO
         MG_PERF(mype+1)%TIME_PACK_Z_PE = MG_PERF(mype+1)%TIME_PACK_Z_PE + MPI_WTIME() - &
                                  TIME_START_DIR
      ...
      IF ( linfo(mype+1)%NEIGHBORS(NORTH) /= -1 ) THEN
         ...
         MG_PERF(mype+1)%TIME_PACK_Y_PE = MG_PERF(mype+1)%TIME_PACK_Y_PE + MPI_WTIME() - &
                                  TIME_START_DIR
      	call SSTMAC_compute_loop2(0, NX+1, 0, NZ+1, 2)
      END IF
