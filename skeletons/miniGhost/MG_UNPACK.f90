...
CONTAINS
   ...
   SUBROUTINE MG_UNPACK ( IERR, mype )
     ...
     !    GRID
      ...
      DO L = 1, linfo(mype+1)%NUM_RECVS + linfo(mype+1)%NUM_SENDS
         ...
         CALL CHECK_ERROR ( IERR, 'MG_UNPACK: MPI_WAITANY', ISTAT(MPI_ERROR), mype )
         ...
         ELSE 
            ...
            OFFSET = 0
            CALL GET_FACE ( IWHICH, OFFSET,  IERR, mype )
      ...
      CALL CHECK_ERROR ( NUM_SENDS_OUTSTANDING, 'MG_UNPACK: NUM_SENDS_OUTSTANDING',  &
                         linfo(mype+1)%NUM_SENDS, mype )
      CALL CHECK_ERROR ( NUM_RECVS_OUTSTANDING, 'MG_UNPACK: NUM_RECVS_OUTSTANDING',  &
   ...
   END SUBROUTINE MG_UNPACK
   
END MODULE MG_UNPACK_MOD
