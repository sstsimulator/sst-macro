...
CONTAINS
   ...
   SUBROUTINE MG_ISEND ( IERR, mype )
      ...
      ! ---------------------
  ! WRITE(*,*) 'mini_ghost(',linfo(mype+1)%MYPE,'): MG_ISEND'
      IERR = 0
      ...
      MSG_REQS_OFFSET = linfo(mype+1)%MAX_NUM_RECVS
      ...
      IF ( linfo(mype+1)%NEIGHBORS(BACK) /= -1 ) THEN
         ...
         MSG_REQS_OFFSET = linfo(mype+1)%MAX_NUM_RECVS + BACK
         CALL MPI_ISEND (  linfo(mype+1)%COUNT_SEND_BACK, MG_MPI_REAL, &
                          linfo(mype+1)%NEIGHBORS(BACK), linfo(mype+1)%MSG_TAGS(BACK),                &
                          linfo(mype+1)%MPI_COMM_MG, linfo(mype+1)%MSG_REQS(MSG_REQS_OFFSET), IERR )
      ...
      IF ( linfo(mype+1)%NEIGHBORS(FRONT) /= -1 ) THEN
         TIME_START_DIR = MPI_WTIME()
         linfo(mype+1)%NUM_SENDS = linfo(mype+1)%NUM_SENDS + 1
         MSG_REQS_OFFSET = linfo(mype+1)%MAX_NUM_RECVS + FRONT
         CALL MPI_ISEND ( linfo(mype+1)%COUNT_SEND_FRONT, MG_MPI_REAL, &
      ...
      IF ( linfo(mype+1)%NEIGHBORS(EAST) /= -1 ) THEN
         TIME_START_DIR = MPI_WTIME()
         linfo(mype+1)%NUM_SENDS = linfo(mype+1)%NUM_SENDS + 1
         MSG_REQS_OFFSET = linfo(mype+1)%MAX_NUM_RECVS + EAST
         CALL MPI_ISEND (  linfo(mype+1)%COUNT_SEND_EAST, MG_MPI_REAL, &
      ...
      IF ( linfo(mype+1)%NEIGHBORS(WEST) /= -1 ) THEN
         TIME_START_DIR = MPI_WTIME()
         linfo(mype+1)%NUM_SENDS = linfo(mype+1)%NUM_SENDS + 1
         MSG_REQS_OFFSET = linfo(mype+1)%MAX_NUM_RECVS + WEST
         CALL MPI_ISEND (  linfo(mype+1)%COUNT_SEND_WEST, MG_MPI_REAL, &
      ...
      IF ( linfo(mype+1)%NEIGHBORS(NORTH) /= -1 ) THEN
         TIME_START_DIR = MPI_WTIME()
         linfo(mype+1)%NUM_SENDS = linfo(mype+1)%NUM_SENDS + 1
         MSG_REQS_OFFSET = linfo(mype+1)%MAX_NUM_RECVS + NORTH
         CALL MPI_ISEND (  linfo(mype+1)%COUNT_SEND_NORTH, MG_MPI_REAL, &
      ...
      IF ( linfo(mype+1)%NEIGHBORS(SOUTH) /= -1 ) THEN
         TIME_START_DIR = MPI_WTIME()
         linfo(mype+1)%NUM_SENDS = linfo(mype+1)%NUM_SENDS + 1
         MSG_REQS_OFFSET = linfo(mype+1)%MAX_NUM_RECVS + SOUTH
         CALL MPI_ISEND (  linfo(mype+1)%COUNT_SEND_SOUTH, MG_MPI_REAL, &
   ...
   END SUBROUTINE MG_ISEND
   
END MODULE MG_ISEND_MOD
