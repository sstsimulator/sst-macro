...
MODULE MG_SVCP_3D7PT_MOD
   ...
   !
   ! 1) FRONT and BACK linfo(mype+1)%NEIGHBORS: X-Y faces, being contiguous in memory, are transmitted
   !    as a single message. In order to avoid intermediate buffering, ghost data is
   !    included in the message.
   ! 2) EAST and WEST linfo(mype+1)%NEIGHBORS: Each column of the Y-Z face is contiguous in memory,
   !    and therefore transmitted as soon as available. That is, NZ messages of length NY 
   !    are transmitted. 
   ! 3) NORTH and SOUTH linfo(mype+1)%NEIGHBORS: No cell of the X-Z face is contiguous in memory.
...
CONTAINS
   ...
   SUBROUTINE MG_SVCP_3D7PT (  GRID_OUT, IVAR, IERR, mype )
     ...
     !    GRID_IN
      ...
      IF ( linfo(mype+1)%NEIGHBORS(BACK) /= -1 ) THEN
         ...
         CALL MPI_IRECV (  (NX+2)*(NY+2), MG_MPI_REAL, linfo(mype+1)%NEIGHBORS(BACK),  &
                          linfo(mype+1)%MSG_TAGS(FRONT), linfo(mype+1)%MPI_COMM_MG, linfo(mype+1)%MSG_REQS(MSG_REQS_OFFSET), IERR )
         CALL CHECK_ERROR ( IERR, 'MG_SVCP_3D7PT: MPI_IRECV(GRID_OUT(BACK))', 1, mype )
      ...
      IF ( linfo(mype+1)%NEIGHBORS(FRONT) /= -1 ) THEN
         ...
         CALL CHECK_ERROR ( IERR, 'MG_SVCP_3D7PT: MPI_IRECV(GRID_OUT(FRONT))', 1, mype )
         MG_PERF(mype+1)%TIME_RECV_Z_PE = MG_PERF(mype+1)%TIME_RECV_Z_PE + MPI_WTIME() - &
                                  TIME_START_DIR
      ...
      IF ( linfo(mype+1)%NEIGHBORS(EAST) /= -1 ) THEN
         ...
         DO K = 2, NZ - 1
            ...
            CALL MPI_IRECV (  NY+2, MG_MPI_REAL, linfo(mype+1)%NEIGHBORS(EAST),  &
                             ...
                             linfo(mype+1)%MSG_REQS(MSG_REQS_OFFSET), IERR )
            CALL CHECK_ERROR ( IERR, 'MG_SVCP_3D7PT: MPI_IRECV(GRID_OUT(EAST))', K, mype )
      ...
      IF ( linfo(mype+1)%NEIGHBORS(WEST) /= -1 ) THEN
         ...
         DO K = 2, NZ - 1
            ...
            CALL MPI_IRECV (  NY+2, MG_MPI_REAL, linfo(mype+1)%NEIGHBORS(WEST),  &
                             ...
                             linfo(mype+1)%MSG_REQS(MSG_REQS_OFFSET), IERR )
            CALL CHECK_ERROR ( IERR, 'MG_SVCP_3D7PT: MPI_IRECV(GRID_OUT(WEST))', K, mype )
            MSG_REQS_OFFSET = MSG_REQS_OFFSET + 1
      ...
      IF ( linfo(mype+1)%NEIGHBORS(NORTH) /= -1 ) THEN
         ...
         DO K = 2, NZ - 1
            ...
            linfo(mype+1)%NEIGHBORS(NORTH),  &
                             ...
                             linfo(mype+1)%MSG_REQS(MSG_REQS_OFFSET), IERR )
            CALL CHECK_ERROR ( IERR, 'MG_SVCP_3D7PT: MPI_IRECV(NORTH)', K, mype )
         ...
         DO K = 2, NZ - 1
            ...
            CALL MPI_IRECV (  NX, MG_MPI_REAL, &
            	 linfo(mype+1)%NEIGHBORS(SOUTH),  &
                             ...
                             linfo(mype+1)%MSG_REQS(MSG_REQS_OFFSET), IERR )
            CALL CHECK_ERROR ( IERR, 'MG_SVCP_3D7PT: MPI_IRECV(SOUTH)', K, mype )
      ...
      DO K = 1, NZ 
         ...
         DO J = 1, NY
            DO I = 1, NX
              ...
              ! GRID_OUT(I,J,K) =           ( GRID_IN(I,J,K-1)    +                    &
              !                                  GRID_IN(I-1,J,K) +                    &
              !               GRID_IN(I,J-1,K) + GRID_IN(I  ,J,K) + GRID_IN(I,J+1,K)   &
              !                                + GRID_IN(I+1,J,K) +                    &
              !                                     GRID_IN(I,J,K+1) ) * SEVENTH

            END DO
         END DO

         call SSTMAC_compute_loop2(1, NY, 1, NX, 7)
         ...
         IF ( K == 1 ) THEN
            IF ( linfo(mype+1)%NEIGHBORS(BACK) /= -1 ) THEN
               ...
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_3D7PT: MPI_SEND(BACK)', I, mype )
         ...
         ELSE IF ( K == NZ ) THEN
            IF ( linfo(mype+1)%NEIGHBORS(FRONT) /= -1 ) THEN
               ...
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_3D7PT: MPI_SEND(FRONT)', I, mype )
               MG_PERF(mype+1)%TIME_SEND_Z_PE = MG_PERF(mype+1)%TIME_SEND_Z_PE + MPI_WTIME() - &
                                        TIME_START_DIR
         ...
         ELSE
            IF ( linfo(mype+1)%NEIGHBORS(EAST) /= -1 ) THEN
               ...
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_3D7PT: MPI_SEND(EAST)', I, mype )
            ...
            IF ( linfo(mype+1)%NEIGHBORS(WEST) /= -1 ) THEN
               ...
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_3D7PT: MPI_SEND(WEST)', I, mype )
               MG_PERF(mype+1)%TIME_SEND_X_PE = MG_PERF(mype+1)%TIME_SEND_X_PE + MPI_WTIME() - &
                                        TIME_START_DIR
            ...
            IF ( linfo(mype+1)%NEIGHBORS(NORTH) /= -1 ) THEN
               ...
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_3D7PT: MPI_SEND(NORTH)', I, mype )
            ...
            IF ( linfo(mype+1)%NEIGHBORS(SOUTH) /= -1 ) THEN
               ...
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_3D7PT: MPI_SEND(SOUTH)', I, mype )
               MG_PERF(mype+1)%TIME_SEND_Y_PE = MG_PERF(mype+1)%TIME_SEND_Y_PE + MPI_WTIME() - &
                                        TIME_START_DIR
      ...
      call SSTMAC_compute_loop(1, NZ, NX + 8)
      ...
      ! FIXME rbarrett: Can this be a WAITALL? Would that require linfo(mype+1)%MSG_REQS all "real", ie not MPI_NULL?
      DO L = 1, linfo(mype+1)%NUM_RECVS
         TIME_START_DIR = MPI_WTIME()
         CALL MPI_WAITANY ( MSG_REQS_LEN, linfo(mype+1)%MSG_REQS, IWHICH, ISTAT, IERR )
         CALL CHECK_ERROR ( IERR, 'MG_SVCP_3D7PT: MPI_WAITANY', ISTAT(MPI_ERROR), mype )
      ...
      END DO
      CALL CHECK_ERROR ( NUM_RECVS_OUTSTANDING, 'MG_SVCP_3D7PT: NUM_RECVS_OUTSTANDING',  &
                         linfo(mype+1)%NUM_RECVS, mype )

      MG_PERF(mype+1)%TIME_WAIT_PE = MG_PERF(mype+1)%TIME_WAIT_PE + MPI_WTIME ( ) - TIME_START

!     CALL MPI_WAITALL ( MSG_REQS_LEN, linfo(mype+1)%MSG_REQS, ISTAT_ALL, IERR )
!     CALL CHECK_ERROR ( IERR, 'MG_SVCP_3D7PT: MPI_WAITALL', IERR )
   ...
   END SUBROUTINE MG_SVCP_3D7PT
   
END MODULE MG_SVCP_3D7PT_MOD
