...
CONTAINS
   ...
   SUBROUTINE MG_BSEND_SVAF (  IERR, mype )
     ...
     !    GRID
      ...
      IF ( linfo(mype+1)%NEIGHBORS(BACK) /= -1 ) THEN
         ...
         linfo(mype+1)%COUNT_SEND_BACK = (NX+1)*(NY+1)
      ...
      IF ( linfo(mype+1)%NEIGHBORS(FRONT) /= -1 ) THEN
         TIME_START_DIR = MPI_WTIME()
         linfo(mype+1)%COUNT_SEND_FRONT = (NX+1)*(NY+1)
         linfo(mype+1)%NUM_SENDS = linfo(mype+1)%NUM_SENDS + 1
   ...
   END SUBROUTINE MG_BSEND_SVAF
   
END MODULE MG_BSEND_SVAF_MOD
