...
CONTAINS
   ...
   SUBROUTINE MG_SVCP_DIAGS_INIT ( IVAR, IERR, mype )
     ...
     !    GRID
      ...
      IF ( linfo(mype+1)%NEIGHBORS(BACK) /= -1 ) THEN
         ...
         CALL CHECK_ERROR ( IERR, 'MG_SVCP_DIAGS_INIT: MPI_IRECV(BACK)', 1, mype )
      ...
      IF ( linfo(mype+1)%NEIGHBORS(FRONT) /= -1 ) THEN
         ...
         linfo(mype+1)%NEIGHBORS(FRONT),  &
                          linfo(mype+1)%MSG_TAGS(BACK), linfo(mype+1)%MPI_COMM_MG, &
                           linfo(mype+1)%MSG_REQS(MSG_REQS_OFFSET), IERR )
         CALL CHECK_ERROR ( IERR, 'MG_SVCP_DIAGS_INIT: MPI_IRECV(FRONT)', 1, mype )
         ...
         DO K = 2, NZ - 1
            ...
            linfo(mype+1)%NEIGHBORS(EAST),  &
                             linfo(mype+1)%MSG_TAGS(WEST)+K, linfo(mype+1)%MPI_COMM_MG, &
                              linfo(mype+1)%MSG_REQS(MSG_REQS_OFFSET), IERR )
            CALL CHECK_ERROR ( IERR, 'MG_SVCP_DIAGS_INIT: MPI_IRECV(EAST)', K, mype )
            ...
            CALL MPI_IRECV (  NY, MG_MPI_REAL, &
            	linfo(mype+1)%NEIGHBORS(WEST),  &
                             linfo(mype+1)%MSG_TAGS(EAST)+K, linfo(mype+1)%MPI_COMM_MG, &
                              linfo(mype+1)%MSG_REQS(MSG_REQS_OFFSET), IERR )
            CALL CHECK_ERROR ( IERR, 'MG_SVCP_DIAGS_INIT: MPI_IRECV(WEST)', K, mype )
            MSG_REQS_OFFSET = MSG_REQS_OFFSET + 1
            OFFSET = OFFSET + NY
         ...
         DO K = 2, NZ - 1
            ...
            CALL MPI_IRECV (  NX, MG_MPI_REAL, &
            	 linfo(mype+1)%NEIGHBORS(NORTH),  &
                             ...
                             linfo(mype+1)%MSG_REQS(MSG_REQS_OFFSET), IERR )
            CALL CHECK_ERROR ( IERR, 'MG_SVCP_DIAGS_INIT: MPI_IRECV(NORTH)', K, mype )
      ...
      IF ( linfo(mype+1)%NEIGHBORS(SOUTH) /= -1 ) THEN
         ...
         DO K = 2, NZ - 1
            ...
            linfo(mype+1)%NEIGHBORS(SOUTH),  &
                             ...
                             linfo(mype+1)%MSG_REQS(MSG_REQS_OFFSET), IERR )
            CALL CHECK_ERROR ( IERR, 'MG_SVCP_DIAGS_INIT: MPI_IRECV(SOUTH)', K, mype )
      ...
      DO K = 0, NZ+1
         ...
         IF ( K == 1 ) THEN
            IF ( linfo(mype+1)%NEIGHBORS(BACK) /= -1 ) THEN
               ...
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_DIAGS_INIT: MPI_SEND(BACK)', I, mype )
         ...
         ELSE IF ( K == NZ ) THEN
            IF ( linfo(mype+1)%NEIGHBORS(FRONT) /= -1 ) THEN
               ...
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_DIAGS_INIT: MPI_SEND(FRONT)', I, mype )
         ...
         ELSE
            IF ( linfo(mype+1)%NEIGHBORS(EAST) /= -1 ) THEN
               ...
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_DIAGS_INIT: MPI_SEND(EAST)', I, mype )
            ...
            IF ( linfo(mype+1)%NEIGHBORS(WEST) /= -1 ) THEN
               ...
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_DIAGS_INIT: MPI_SEND(WEST)', I, mype )
            ...
            IF ( linfo(mype+1)%NEIGHBORS(NORTH) /= -1 ) THEN
               ...
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_DIAGS_INIT: MPI_SEND(NORTH)', I, mype )
            ...
            IF ( linfo(mype+1)%NEIGHBORS(SOUTH) /= -1 ) THEN
               ...
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_DIAGS_INIT: MPI_SEND(SOUTH)', I, mype )
      ...
      DO L = 1, linfo(mype+1)%NUM_RECVS
         ...
         CALL CHECK_ERROR ( IERR, 'MG_SVCP_DIAGS_INIT: MPI_WAITANY', ISTAT(MPI_ERROR), mype )
      
         NUM_RECVS_OUTSTANDING = NUM_RECVS_OUTSTANDING - 1
      
      END DO
      CALL CHECK_ERROR ( NUM_RECVS_OUTSTANDING, 'MG_SVCP_DIAGS_INIT: NUM_RECVS_OUTSTANDING',  &
                         linfo(mype+1)%NUM_RECVS, mype )

!     CALL MPI_WAITALL ( MSG_REQS_LEN, MSG_REQS, ISTAT_ALL, IERR )
!     CALL CHECK_ERROR ( IERR, 'MG_SVCP_DIAGS_INIT: MPI_WAITALL', IERR )
      ...
      IF ( linfo(mype+1)%NEIGHBORS(NORTH) /= -1 ) THEN
         ...
         CALL CHECK_ERROR ( IERR, 'MG_SVCP_DIAGS_INIT: Unpacking NORTH face', IVAR, mype )
      END IF
      
      IF ( linfo(mype+1)%NEIGHBORS(SOUTH) /= -1 ) THEN
         OFFSET = 0
         CALL MG_GET_FACE ( IVAR, SOUTH, OFFSET, IERR, mype )
         CALL CHECK_ERROR ( IERR, 'MG_SVCP_DIAGS_INIT: Unpacking SOUTH face', IVAR, mype )
      END IF

      linfo(mype+1)%NUM_RECVS = NUM_RECVS_OUTSTANDING

   END SUBROUTINE MG_SVCP_DIAGS_INIT
   
END MODULE MG_SVCP_DIAGS_INIT_MOD
