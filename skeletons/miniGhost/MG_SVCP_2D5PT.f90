...
CONTAINS
   ...
   SUBROUTINE MG_SVCP_2D5PT (  IERR )
     ...
     !    GRID
      ...
      IF ( MYPE == ROOT_PE ) &
         WRITE(*,*) 'MG_SVCP_2D5PT not yet implemented'
      ...
      DO K = 1, NZ 
         ...
         call SSTMAC_compute_loop2(1, NY, 1, NX, 5)

         IF ( K == 1 ) THEN
            IF ( NEIGHBORS(BACK) /= -1 ) THEN
               CALL MPI_SEND ( NX*NY, MG_MPI_REAL, NEIGHBORS(BACK), MSG_TAGS(BACK),  &
                               MPI_COMM_MG, IERR )
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_2D5PT: MPI_SEND(BACK)', I )
         ...
         ELSE IF ( K == NZ ) THEN
            IF ( NEIGHBORS(FRONT) /= -1 ) THEN
               CALL MPI_SEND ( NX*NY, MG_MPI_REAL, NEIGHBORS(FRONT), MSG_TAGS(FRONT),  &
                               MPI_COMM_MG, IERR )
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_2D5PT: MPI_SEND(FRONT)', I )
         ...
         ELSE
            IF ( NEIGHBORS(EAST) /= -1 ) THEN
               CALL MPI_SEND ( NY, MG_MPI_REAL, NEIGHBORS(EAST), MSG_TAGS(EAST)+I,        &
                               MPI_COMM_MG, IERR )
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_2D5PT: MPI_SEND(EAST)', I )
            ...
            IF ( NEIGHBORS(WEST) /= -1 ) THEN
               CALL MPI_SEND ( NY, MG_MPI_REAL, NEIGHBORS(WEST), MSG_TAGS(WEST)+I,       &
                               MPI_COMM_MG, IERR )
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_2D5PT: MPI_SEND(WEST)', I )
            ...
            IF ( NEIGHBORS(NORTH) /= -1 ) THEN
               ...
               CALL MPI_SEND ( NX, MG_MPI_REAL, NEIGHBORS(NORTH), MSG_TAGS(NORTH)+K,                &
                               MPI_COMM_MG, IERR )
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_2D5PT: MPI_SEND(NORTH)', I )
            ...
            IF ( NEIGHBORS(SOUTH) /= -1 ) THEN
               ...
               CALL MPI_SEND ( NX, MG_MPI_REAL, NEIGHBORS(SOUTH), MSG_TAGS(SOUTH)+K,  &
                               MPI_COMM_MG, IERR )
               CALL CHECK_ERROR ( IERR, 'MG_SVCP_2D5PT: MPI_SEND(SOUTH)', I )
   ...
   END SUBROUTINE MG_SVCP_2D5PT
   
END MODULE MG_SVCP_2D5PT_MOD
