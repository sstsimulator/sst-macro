...
, blk_mem_size_(0)
{
  using std::cout;
  using std::endl;
  using std::runsprockit::time_error;
  ...
  if (numThreads_ != 64 && numThreads_ != 128 && numThreads_ != 256 && numThreads_ != 512
      ...
      && numThreads_ != 32) {
//    throw runsprockit::time_error("CUDANode::CUDANode(): number of threads per block must be a power of two in [1,512].");
  }
  int deviceCount; cudaGetDeviceCount(&deviceCount); 
  if (device >= deviceCount) {
    if (deviceCount == 0) {
//      throw runsprockit::time_error("CUDANode::CUDANode(): system has no CUDA devices.");
    }
    if (verbose) {
