...
mpi_api::precommit_types()
{
  ...
  if (!mpi_type::mpi_null->committed()){
    mpi_type::mpi_null->init_primitive("MPI_NULL", 0, 0);
    ...
    mpi_type::mpi_wchar->init_primitive("MPI_WCHAR", 2, 4);

    mpi_type::mpi_unsigned_long_long->init_primitive("MPI_UNSIGNED_LONG_LONG", 8, 8);

    mpi_type::mpi_lb->init_primitive("MPI_LB", 0, 0);

    mpi_type::mpi_ub->init_primitive("MPI_UB", 0, 0);

    mpi_type::mpi_unsigned_char->init_primitive("MPI_UNSIGNED_CHAR", 1, 4);

    mpi_type::mpi_byte->init_primitive("MPI_BYTE", 1, 4);

    mpi_type::mpi_short->init_primitive("MPI_SHORT", 2, 4);
    ...
    mpi_type::mpi_double->init_primitive("MPI_DOUBLE", 8, 8);
    ...
    mpi_type::mpi_real4->init_primitive("MPI_REAL4", 4, 4);

    mpi_type::mpi_real8->init_primitive("MPI_REAL8", 8, 8);

    mpi_type::mpi_real16->init_primitive("MPI_REAL16", 16, 16);
    ...
    mpi_type::mpi_complex16->init_primitive("MPI_COMPLEX16", mpi_type::mpi_double,
                                       mpi_type::mpi_double, sizeof(dcomplex));

    mpi_type::mpi_complex32->init_primitive("MPI_COMPLEX32",
                                       mpi_type::mpi_long_double, mpi_type::mpi_long_double,
                                       sizeof(ldcomplex));
    ...
    for (int i=0; i < num_builtins; ++i){
      ...
      mpi_type::builtins[size].init_primitive(label, size, 0);
    }
  }
                      ...
                      MPI_Datatype intype, MPI_Datatype* outtype)
{
  ...
  return type_indexed(count, lens, new_ind, intype, outtype);
}
