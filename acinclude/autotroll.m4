...
[AC_DEFUN([AX_INSTEAD_IF],
  [m4_ifval([$1],
    AC_MSG_WARN([Qt 4 not found; control GUI will not be built.])
		AC_MSG_WARN([See http://www.trolltech.com/ to obtain Qt 4.]))])])
...
dnl Memo: AC_ARG_WITH(package, help-string, [if-given], [if-not-given])
  AC_ARG_WITH([qt],
              [AS_HELP_STRING([--with-qt],
                 [Path to Qt @<:@Look in PATH and /usr/bin/qt4@:>@])],
              [QT_PATH=$withval])
  ...
  if test -d /usr/bin/qt4; then
    # Try to find the latest version.
    tmp_qt_paths=`echo /usr/bin/qt4/*/bin | tr ' ' '\n' | sort -nr \
                                              | xargs | sed 's/  */:/g'`
  fi
...
instead" >&AS_MESSAGE_LOG_FD
  ...
  AC_MSG_RESULT([$QT_VERSION_MAJOR])
