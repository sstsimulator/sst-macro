...
AC_ARG_ENABLE(sstmac-debug,
  [AS_HELP_STRING(
    ...
    [Whether to enable debug printing in code. Disabling optimizes code slightly by removing many if statements [default=enable]]
    )],
  [
...
AC_ARG_ENABLE(sanity-check,
  [AS_HELP_STRING(
    ...
    [Controls whether safe mode is run with sanity checks. If enabled, code may run slower [default=disable]],
   )],
  [
...
AC_ARG_ENABLE(debug-swap,
  [AS_HELP_STRING(
    ...
    [Whether or not to provide hooks for manual context switching in GDB [default=disable]]
    )],
  [
