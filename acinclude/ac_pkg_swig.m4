...
AC_DEFUN([AC_PROG_SWIG],[
        ...
        if test -z "$SWIG" ; then
                AC_MSG_WARN([cannot find 'swig'. You should look at http://www.swig.org])
                SWIG='X'
        elif test -n "$1" ; then
                ...
                if test -n "$swig_version" ; then
                        ...
                        if test $available_major -lt $required_major \
                                ...
                                AC_MSG_WARN([SWIG version $swig_version is too old.  Version >= $1 is required.])
                                SWIG=':'
                        else
        ...
        dnl --- added for sst/macro config reporting ---
        HAVE_SWIG="yes"
        dnl --- end sst/macro additions ---
])
