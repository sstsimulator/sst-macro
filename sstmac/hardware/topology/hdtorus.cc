  ...
  connectable* minus_partner)
{
  ...
  center->connect_weighted(
    pos_outport, neg_outport,
    connectable::output,
    plus_partner, 1.0, red_[dim]);
...
void
hdtorus::display_nodes(const vis_switch_map &switches,
                       vis::vis_engine* eng,
                       std::list<vis::vis_obj*> &objs)
{
  for (long i = 0; i < static_cast<long>(switches.size()); ++i) {
    switch_id me(i);

    std::vector<int> coords = switch_coords(me);
    ...
    const vis_switch_map::mapped_type& myswitch = mapget(switches, me,
        "network switch for hdtorus");
    myswitch->set_vis_obj(vo);
    myswitch->set_vis_engine(eng);
    objs.push_back(vo);

    for (unsigned int dim = 0; dim < dimensions_.size(); ++dim) {
      ...
      switch (dim) {
        ...
        default: {
          spkt_throw_printf(sprockit::value_error, "invalid dimension for hdtorus");
          return;
        }
      } // switch (dim)
      ...
      if (coords[dim] != dimensions_[dim] - 1) {
        vis::vis_obj* c1 = eng->create_cylinder(x1, y1, z1, 0.125, 1, dir);
        objs.push_back(c1);
        myswitch->set_link_vis(c1, dim, pos);
      }

      if (coords[dim]) {
        vis::vis_obj* c2 = eng->create_cylinder(x2, y2, z2, 0.125, 1, dir);
        myswitch->set_link_vis(c2, dim, neg);
        objs.push_back(c2);
      }
    } // dim
  }
